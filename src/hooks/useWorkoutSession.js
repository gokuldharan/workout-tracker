...
export function useWorkoutSession(dayId) {
  ...
  useEffect(() => {
    ...
    async function init() {
      ...
      // Fetch exercise details and last sessions
      const [lastSessions] = await Promise.all([
        getLastSessionsForExercises(exerciseIds),
      ])

      // Build exercise lookup from allExercises (may not be loaded yet)
      ...
      const exercises = exerciseIds.map((id) => {
        ...
        // Pre-fill with last session's values or 3 empty sets
        const sets = lastSets.length
          ? lastSets.map((s) => ({ r: s.r, w: s.w }))
          : [{ r: 10, w: 0 }, { r: 10, w: 0 }, { r: 10, w: 0 }]
      ...
      return {
        ...
        exercises: [
          ...prev.exercises,
          {
            ...
            sets: [{ r: 10, w: 0 }, { r: 10, w: 0 }, { r: 10, w: 0 }],
            lastSets: [],
            done: false,
